//===============
// YOMI PARSING ENGINE - REVOLUTION V2
// Optimized for strict season isolation and absolute numbering support.
//===============
...
function extractEpisodeNumber(filename, expectedSeason = 1) {
    ...
    // 1. Strict Season & Episode Match (z.B. S01E16)
...
function isEpisodeMatch(name, requestedEp, expectedSeason = 1) {
    ...
    if (extractedEp === -1) return false; // Falsche Staffel
    if (extractedEp !== null) return extractedEp === epNum;
    
    // PRIORITY 3: PILOT FALLBACK (Only for Episode 1)
...
function selectBestVideoFile(files, requestedEp, expectedSeason = 1) {
    ...
    // Strict fallback for single-file pilot episodes
