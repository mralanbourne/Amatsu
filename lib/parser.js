...
function extractEpisodeNumber(filename, expectedSeason = 1) {
    ...
    const explicitRegex = /(?:ep(?:isode)?\.?\s*|ova\s*|oad\s*|special\s*|round\s*|act\s*|chapter\s*|vol(?:ume)?\.?\s*|第\s*|s(\d+)\s*e|season\s*(\d+)\s*ep(?:isode)?\s*)0*(\d+)(?:\s*(?:巻|話))?(?:\D|$)/i;
