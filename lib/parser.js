...
function selectBestVideoFile(files, requestedEp, expectedSeason = 1, isMovie = false) {
    ...
    //===============
    // SABOTAGE ENTFERNT: isMovie wird nicht mehr fälschlicherweise auf false gesetzt, 
    // nur weil Samples oder Trailer im Torrent sind.
    //===============
    if (epNum > 1) {
        isMovie = false;
    }
    ...
    if (!isMovie) {
        videoFiles = videoFiles.filter(f => {
            ...
            //===============
            // LIMIT ERHÖHT: 20GB für Episoden zulassen.
            //===============
