...
async function searchNyaaForAnime(romajiTitle) {
    ...
    const fetchPromise = (async () => {
        ...
        const agent = new SocksProxyAgent("socks5h://warp-proxy:1080");
        for (const baseUrl of MIRRORS) {
            try {
                const rssUrl = `${baseUrl}/?q=${encodedQuery}&s=seeders&o=desc&c=1_0`;
                console.log(`[NYAA NETWORK] Searching via Tor (${baseUrl}): ${romajiTitle}`);
                ...
                if (typeof response.data === "string" && response.data.trim().startsWith("<!DOCTYPE html>")) {
                    console.warn(`[NYAA WARN] Cloudflare Captcha block on ${baseUrl}, trying next mirror...`);
                    continue;
                }
                ...
                console.warn(`[NYAA WARN] Mirror ${baseUrl} failed via Tor: ${error.message}`);

            }
        }

        console.error(`[NYAA ERROR] All mirrors failed via Tor for: ${romajiTitle}`);
        return [];
    })();
