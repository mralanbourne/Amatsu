...
// RESTORED: Uses standard "anilist:" prefixes for native Stremio compatibility.
// Includes :v2 Cache-Buster and advanced Episode Title/Date extraction.
//===============
...
// Polyfill for base64url encoding
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                const results = response.data.data.Page.media.map(anime => {
                    ...
                    return {
                        id: "anilist:" + anime.id + ":" + base64Title + ":v2",
                        type: "series", 
                        name: cleanTitle,
...
async function getAnimeMeta(anilistId, retries = 3) {
    ...
    // Added idMal and streamingEpisodes to extract rich titles and thumbnails
    const graphqlQuery = `
        query ($id: Int) { 
    ...
    const fetchPromise = (async () => {
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                // Parse streaming episodes to acquire native thumbnails and title formats
                let epMeta = {};
...
// Pulls explicit episode titles and air dates from Jikan (MyAnimeList) API
async function fetchEpisodeDetails(malId) {
    ...
    const eps = {};
    try {
        // Fetching page 1 (Eps 1-100) covers the majority of active or short series without rate-limiting
        const res = await axios.get(`https://api.jikan.moe/v4/anime/${malId}/episodes`, { timeout: 4000 });
...
async function getJikanMeta(cleanedTitle, retries = 3) {
    ...
    const fetchPromise = (async () => {
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                const url = "https://api.jikan.moe/v4/anime?q=" + encodeURIComponent(cleanedTitle) + "&sfw=true&limit=1";
...
module.exports = { searchAnime, getAnimeMeta, getTrendingAnime, getTopAnime, getJikanMeta, fetchEpisodeDetails };
