...
async function getAnimeMeta(anilistId, retries = 3) {
    ...
    // Fetches 'airingAt' timestamp to enable accurate reverse-math calculations
    const graphqlQuery = `
        query ($id: Int) { 
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                const result = {
                    ...
                    epMeta: epMeta,
                    baseTime: baseTime,
                    // Injecting nextAiringEpisode to use its timestamp for reverse-math calculation
...
async function fetchEpisodeDetails(malId) {
    ...
    try {
        ...
        // STRICT SAFETY CAP: Limited to 4 pages (400 episodes) max.
        // Fetching more takes > 5 seconds and will trigger a hard 504 Timeout in the Stremio Client.
