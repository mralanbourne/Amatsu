        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                const results = response.data.data.Page.media.map(anime => {
                    ...
                    const stremioType = (anime.format === "MOVIE" || anime.format === "MOVIE_PREMIERE") ? "movie" : "series";
...
async function getTrendingAnime(stremioType = "series") {
    const formats = stremioType === "movie" ? ["MOVIE", "MOVIE_PREMIERE"] : ["TV", "TV_SHORT", "ONA", "OVA", "SPECIAL"];
    const graphqlQuery = `
