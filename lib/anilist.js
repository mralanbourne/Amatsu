        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                const results = response.data.data.Page.media.map(anime => {
                    ...
                    //===============
                    const stremioType = anime.format === "MOVIE" ? "movie" : "series";
...
//===============
async function getTrendingAnime(stremioType = "series") {
...
async function getTopAnime(stremioType = "series") {
    const formats = stremioType === "movie" ? ["MOVIE"] : ["TV", "TV_SHORT", "ONA", "OVA", "SPECIAL"];
    const graphqlQuery = `
        query ($sort: [MediaSort], $formats: [MediaFormat]) { 
            Page(page: 1, perPage: 40) { 
...
async function getAiringAnime(stremioType = "series") {
    const formats = stremioType === "movie" ? ["MOVIE"] : ["TV", "TV_SHORT", "ONA", "OVA"];
    const graphqlQuery = `
        query ($sort: [MediaSort], $formats: [MediaFormat], $status: MediaStatus) { 
            Page(page: 1, perPage: 40) { 
    ...
    return _fetchAniList(graphqlQuery, { sort: ["POPULARITY_DESC"], formats: formats, status: "RELEASING" });
}
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    const releaseInfo = year ? "" + year : undefined;
                    const released = year ? new Date(Date.UTC(year, (month || 1) - 1, day || 1)).toISOString() : undefined;
                    const stremioType = (anime.type === "Movie") ? "movie" : "series";
...
module.exports = { searchAnime, getAnimeMeta, getTrendingAnime, getTopAnime, getAiringAnime, getJikanMeta, fetchEpisodeDetails };
