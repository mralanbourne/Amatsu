//===============
// YOMI DEBRID PROVIDER INTERFACE (WARP ENABLED)
//===============
...
async function checkRD(hashes, apiKey) {
    ...
    const performFetch = async () => {
        try {
                    ...
                    if (availability && availability.rd && availability.rd.length > 0) {
                        ...
                        availability.rd.forEach(variant => {
                            Object.keys(variant).forEach(fileId => {
                                if (!allFilesMap.has(fileId)) allFilesMap.set(fileId, { id: fileId, name: variant[fileId].filename, size: variant[fileId].filesize });
                            });
                        });
            ...
            return { data: {}, ttl: (status === 401 || status === 403) ? 3600000 : 30000 }; 
        }
    };
    ...
    const performFetch = async () => {
        try {
            ...
            for (let i = 0; i < hashes.length; i += 40) {
                ...
                const res = await axios.get(url, { headers: { "Authorization": "Bearer " + apiKey }, httpsAgent: agent, timeout: 8000 });
...
async function getActiveRD(apiKey) {
    ...
    const performFetch = async () => {
        try {
            const agent = new SocksProxyAgent("socks5h://warp-proxy:9091");
            const res = await axios.get("https://api.real-debrid.com/rest/1.0/torrents?limit=100", { headers: { "Authorization": "Bearer " + apiKey }, httpsAgent: agent, timeout: 8000 });
            const active = {};
            res.data.forEach(t => {
...
async function getActiveTorbox(apiKey) {
    ...
    const performFetch = async () => {
        try {
            const agent = new SocksProxyAgent("socks5h://warp-proxy:9091");
            const res = await axios.get("https://api.torbox.app/v1/api/torrents/mylist?bypass_cache=true", { headers: { "Authorization": "Bearer " + apiKey }, httpsAgent: agent, timeout: 8000 });
            const active = {};
            if (res.data && res.data.data) {
                res.data.data.forEach(t => {
