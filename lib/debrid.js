...
// (Clean Architecture + Strict Rate Limit Compliance)
//===============
...
async function checkRD(hashes, apiKey) {
    ...
    console.log(`[DEBRID FORENSICS] Frage Real-Debrid API für ${hashes.length} Hashes ab...`);
    ...
    const performFetch = async () => {
        try {
            ...
            // FIX: Chunk-Größe von 40 auf 10 reduziert, um lange URLs und 403 Blocks zu vermeiden!
            const chunkSize = 10; 
            ...
            for (let i = 0; i < hashes.length; i += chunkSize) {
                ...
                // Wir senden den API-Key exakt nach Doku-Vorgabe
                ...
                // FIX: Polling Delay erhöht auf 500ms, um "250 requests per minute" Limit NIEMALS zu brechen
...
async function checkTorbox(hashes, apiKey) {
    ...
    const performFetch = async () => {
        try {
            const results = {};
            const chunkSize = 20; // Torbox verträgt etwas mehr
            for (let i = 0; i < hashes.length; i += chunkSize) {
                const chunk = hashes.slice(i, i + chunkSize);
