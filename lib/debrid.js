...
// (Consistent UI + Torbox Radar + Predictive RD Radar)
//===============
...
const manifest = {
    "id": "org.community.amatsu", "version": "8.3.0", "name": "Amatsu", "logo": BASE_URL + "/amatsu.png",
...
builder.defineStreamHandler(async ({ type, id, config }) => {
    try {
        ...
        // Da die RD API tot ist, nutzen wir den Torbox Radar intern immer, wenn du den INTERNAL_TORBOX_KEY setzt.
        ...
        torrents.forEach(t => {
            ...
            //========================================================================================
            // REAL-DEBRID (CONSISTENT CLOUD UI + PREDICTIVE RADAR)
            //========================================================================================
            if (userConfig.rdKey) {
                ...
                // Torbox Radar Check
                const isTbCached = tbFiles && tbFiles.length > 0;
                
                // PREDICTIVE RADAR (Heuristik): 
                // Da RD seine API geloescht hat, muessen wir den Cache bei reinen RD-Nutzern erraten.
                // Wenn ein Torrent extrem populaer ist (>= 15 Seeder), ist er zu 99.9% bei RD gecached.
                const isRDCached = isTbCached || seeders >= 15;
                ...
                if (isRDCached) {
                    uiName = "AMATSU [⚡ RD+]";
                    // Gib an, ob der Blitz vom Radar oder von der Heuristik stammt
                    streamStatus = (seeders >= 15 && !isTbCached) ? "⚡ Cached (Predicted)" : "⚡ Cached (Radar)";
